...
public class Main_category extends Activity {

    TextView subcat_name = null, cart_number = null;
    ListView sub_list = null;
    Bundle bb = null, b = null;
    String Dep_name = null, Dep_id = null, shop_id = null, getStatus = null;

    ProgressDialog pgBar = null;
    GetSet_maincategory getSet_maincategory = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            Intent i = getIntent();
            bb = i.getExtras();
            Dep_name = bb.getString("Dep_name");
            Dep_id = bb.getString("Dep_id");
            shop_id = bb.getString("shop_id");
            pref = getSharedPreferences("Login", MODE_PRIVATE);
            getStatus = pref.getString("username", "");
            subcat_name.setText(Dep_name);
            getting_main_category(Dep_id, shop_id);
            cart_item_no();
            if (getStatus.equals("")) {
    ...
    private void getting_main_category(String dep_id1, String shop_id1) {
        pgBar = Progress_class.createProgressDialog(Main_category.this);
        pgBar.show();
        ApiClient.getPosApp().get_mainCategaorydetails(shop_id1, dep_id1, "", new Callback<GetSet_maincategory>() {
            public void success(GetSet_maincategory cartModel, Response response) {
            ...
            private void consumeApiData1(GetSet_maincategory GetSet_subcatt) {
                getSet_maincategory = GetSet_subcatt;
                if (getSet_maincategory.getstatus().equals("true")) {
                    if (getSet_maincategory != null) {
                        try {

                            listAdapter1 = new ListViewAdapter1(getApplicationContext(), getSet_maincategory);
                            sub_list.setAdapter(listAdapter1);
                            sub_list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                @Override
                                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                                    String category_id = getSet_maincategory.getmaincategory().get(position).getcat_id();
                                    String category_name = getSet_maincategory.getmaincategory().get(position).getCat_name();
                                    bb.putString("category_id", category_id);
                                    bb.putString("category_name", category_name);
                                    ...
                                    Intent i = new Intent(getApplicationContext(), Sub_category.class);
                                    i.putExtras(bb);
                                    startActivity(i);
                                    overridePendingTransition(R.anim.slide_in, R.anim.slide_out);
                                }
                ...
                } else if (getSet_maincategory.getstatus().equals("false")) {
                    try {
                        if (pgBar.isShowing()) {
                            pgBar.dismiss();
                        }
    ...
    public class ListViewAdapter1 extends BaseAdapter {
        ...
        public ListViewAdapter1(Context context, GetSet_maincategory getSet_catt1) {
            inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            getSet_maincategory = getSet_catt1;
        ...
        public int getCount() {
            ...
            return getSet_maincategory.getmaincategory().size();
        }
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            try {
                if (!getSet_maincategory.getmaincategory().get(position).getimage().equals("")) {
                    imageLoader.displayImage(ApiClient.url_common + "/" + getSet_maincategory.getmaincategory().get(position).getimage(), sub_image, options,
            ...
            subcat_name.setText(getSet_maincategory.getmaincategory().get(position).getCat_name());
            return itemView;
        }
    }
